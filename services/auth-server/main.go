...
func main() {
	...
	// Project-scoped OAuth admin CRUD
...
func googleLoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	if redirectURI == "" { redirectURI = os.Getenv("RENDER_EXTERNAL_URL") }
	...
	// Embed project ref and redirect URI into the state
	rawState := fmt.Sprintf("%s|%s|%s", ref, redirectURI, base64.URLEncoding.EncodeToString(make([]byte, 16)))
	stateEnc := base64.URLEncoding.EncodeToString([]byte(rawState))
	dbPool.Exec(context.Background(), `INSERT INTO oauth_states (state, provider, project_ref, redirect_uri) VALUES ($1,'google',$2,$3)`, stateEnc, ref, redirectURI)
...
func googleCallbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	var prov, pref, redir string
	_ = dbPool.QueryRow(context.Background(), `SELECT provider, project_ref, redirect_uri FROM oauth_states WHERE state=$1`, state).Scan(&prov, &pref, &redir)
	...
	// Redirect to the original redirect URI with token & userId
	if redir != "" {
		sep := "?"
		if strings.Contains(redir, "?") { sep = "&" }
		http.Redirect(w, r, fmt.Sprintf("%s%stoken=%s&userId=%s", redir, sep, signed, userID), http.StatusFound)
		return
	}
...
func githubLoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	rawState := fmt.Sprintf("%s|%s|%s", ref, redirectURI, base64.URLEncoding.EncodeToString(make([]byte, 16)))
	stateEnc := base64.URLEncoding.EncodeToString([]byte(rawState))
	dbPool.Exec(context.Background(), `INSERT INTO oauth_states (state, provider, project_ref, redirect_uri) VALUES ($1,'github',$2,$3)`, stateEnc, ref, redirectURI)
	http.Redirect(w, r, cfg.AuthCodeURL(stateEnc), http.StatusFound)
}
