...
func createOAuthProviderHandler(w http.ResponseWriter, r *http.Request) {
	var req struct {
		...
		AllowUsersWithoutEmail bool   `json:"allow_users_without_email"`
	...
	dbPool.Exec(context.Background(),
		`INSERT INTO oauth_providers (provider, client_id, client_secret, enabled, skip_nonce_check, allow_users_without_email, callback_url)
		 ...
		 client_id=$2, client_secret=$3, enabled=$4, skip_nonce_check=$5, allow_users_without_email=$6, callback_url=$7`,
		req.Provider, req.ClientID, req.ClientSecret, req.Enabled, req.SkipNonceCheck, req.AllowUsersWithoutEmail, req.CallbackURL)
	loadOAuthConfigs()
	w.Write([]byte(`{"status":"created"}`))
}
func updateOAuthProviderHandler(w http.ResponseWriter, r *http.Request) {
	...
	dbPool.Exec(context.Background(),
		`UPDATE oauth_providers SET
		 client_id=$1, client_secret=$2, enabled=$3, skip_nonce_check=$4, allow_users_without_email=$5, callback_url=$6
		 WHERE provider=$7`,
		req.ClientID, req.ClientSecret, req.Enabled, req.SkipNonceCheck, req.AllowUsersWithoutEmail, req.CallbackURL, provider)
	loadOAuthConfigs()
	w.Write([]byte(`{"status":"updated"}`))
}
...
func oauthLoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Fetch custom callback URL from DB
	var customCB string
	dbPool.QueryRow(context.Background(), `SELECT callback_url FROM oauth_providers WHERE provider=$1`, provider).Scan(&customCB)
	if customCB != "" {
		config.RedirectURL = customCB
	}
	state := make([]byte, 16)
	rand.Read(state)
...
func oauthCallbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	dbPool.QueryRow(context.Background(),
		...
		provider).Scan(&skipNonce, &allowNoEmail, &customCB)
