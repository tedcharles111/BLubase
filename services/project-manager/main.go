...
func main() {
	...
	var err error
	controlDB, err = connectDB(ctx, os.Getenv("DATABASE_URL"))
	...
	// Developer project‑scoped user management
...
// ---------- All the original handler functions ----------
...
func createProjectHandler(w http.ResponseWriter, r *http.Request) {
	...
	ref := make([]byte, 6); rand.Read(ref); refStr := base64.URLEncoding.EncodeToString(ref)[:6]
	...
	controlDB.Exec(context.Background(), `CREATE TABLE IF NOT EXISTS `+tableName+` (
		id UUID PRIMARY KEY DEFAULT gen_random_uuid(), email TEXT UNIQUE, password_hash TEXT, phone TEXT,
...
func listProjectUsersHandler(w http.ResponseWriter, r *http.Request) {
	...
	query := fmt.Sprintf(`SELECT id, email, phone, created_at FROM %s
		UNION ALL SELECT id, email, NULL as phone, created_at FROM public.users_view
...
func projectSignupHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = controlDB.QueryRow(context.Background(), `SELECT id FROM `+tableName+` WHERE email=$1`, req.Email).Scan(&newUserID)
...
func projectLoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	err := controlDB.QueryRow(context.Background(), `SELECT id, password_hash FROM `+tableName+` WHERE email=$1`, req.Email).Scan(&userID, &hashed)
	...
	claims := jwt.MapClaims{"sub": userID, "email": req.Email, "ref": ref, "iat": time.Now().Unix(), "exp": time.Now().Add(24*time.Hour).Unix()}
